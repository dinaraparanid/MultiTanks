...
--   victory screen of particular player is shown.
--   In other cases, typical game field is shown.
--   This version is not differ from renderGame, just an IO wrapper
...
renderGameIO ::
  ...
  -> SystemState -- ^ current system state
  -> IO Picture
renderGameIO firstPlayerPic secondPlayerPic sys = return $ renderGame firstPlayerPic secondPlayerPic sys
...
handleDirectionEvent _ _ state _ _ = return state
...
handleMovementEvent
  ...
  where
    firstPlayerState = (Just p1Dir, Just p1Crds, p1Shot)
    secondPlayerState = (Just p2Dir, Just p2Crds, p2Shot)
    curState = return (GameState [firstPlayerState, secondPlayerState], playerState)
    newP1Crds = crdTransf p1Crds
    updCrdsRequest = GameState.UpdatePosition $ UpdatePositionData 1 newP1Crds
    nextState = wrapP1MovementWithIO
...
handleShootEvent
  ...
  where
    ...
    noShotState = wrapP1CancelShotWithIO
...
handleShootEvent
  ...
  where
    ...
    noShotState = wrapP2CancelShotWithIO
...
handleGameEvents _ _ _ state = return state
...
updateGame eventQueue _ _ _ curState = do
  nextStateMb <- STM.atomically $ STM.tryReadTChan eventQueue
  case nextStateMb of
...
updateGame1_
  ...
  (Left newP1Shot)
  newP2Shot = wrapP1ShotWithIO requestChan curSysStateHolder fP1Dir fP2Dir fP1Crds fP2Crds newP1Shot newP2Shot
...
updateGame1_
  ...
  newP2Shot = wrapP1CancelShotWithIO requestChan curSysStateHolder fP1Dir fP2Dir fP1Crds fP2Crds newP2Shot
    $ GameState.CancelShoot 1

updateGame1_ _ _ fetchedState _ _ = return fetchedState
...
updateGame2_
  ...
  (GameState [(Just fP1Dir, Just fP1Crds, _), (Just fP2Dir, Just fP2Crds, _)], _)
  newP1Shot
  (Left newP2Shot) = wrapP2ShotWithIO
    requestChan curSysStateHolder fP1Dir fP2Dir fP1Crds fP2Crds newP1Shot newP2Shot
...
updateGame2_
  ...
  (GameState [(Just fP1Dir, Just fP1Crds, _), (Just fP2Dir, Just fP2Crds, _)], _)
  newP1Shot
  (Right GameState.OutOfBorders) = wrapP2CancelShotWithIO
    requestChan curSysStateHolder fP1Dir fP2Dir fP1Crds fP2Crds newP1Shot
    $ GameState.CancelShoot 2

updateGame2_ _ _ fetchedState _ _ = return fetchedState
...
launchGame ::
  ...
  -> Picture                        -- ^ second player picture (already rotated and translated)
  -> IO ()
launchGame eventQueue requestChan curSysStateHolder firstPlayerPic secondPlayerPic = Gloss.playIO
  mainWindow
  white
  fps
  initialSystemState
  (renderGameIO firstPlayerPic secondPlayerPic)
